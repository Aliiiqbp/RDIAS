...
def jpeg_compression(image_path, image, min_q, max_q):
    """Apply JPEG compression."""
    q = random.randint(min_q, max_q)
    image.save("temp.jpg", "JPEG", quality=q)
    return Image.open("temp.jpg")


def resize_keep_ratio(image_path, image, min_scale, max_scale):
...
def resize_no_ratio(image_path, image, size):
    """Resize image without keeping the aspect ratio."""
    return image.resize(size)


def add_gaussian_noise(image_path, image, mean, min_std, max_std):
...
def apply_gaussian_blur(image_path, image, min_radius, max_radius):
    """Apply Gaussian blur to the image."""
    radius = np.random.uniform(min_radius, max_radius)
    return image.filter(ImageFilter.GaussianBlur(radius))


def dropout(image_path, image, drop_ratio):
...
def adjust_saturation(image_path, image, factor):
    """Adjust saturation."""
    enhancer = ImageEnhance.Color(image)
    return enhancer.enhance(factor)


def adjust_brightness(image_path, image, factor):
    """Adjust brightness."""
    enhancer = ImageEnhance.Brightness(image)
    return enhancer.enhance(factor)


def adjust_contrast(image_path, image, factor):
    """Adjust contrast."""
    enhancer = ImageEnhance.Contrast(image)
    return enhancer.enhance(factor)


def jpeg2000_compression(image_path, image, min_db, max_db):
    """Apply JPEG2000 compression."""
    q = random.randint(min_db, max_db)
    image.save("temp.jp2", "JPEG2000", quality_mode='dB', quality_layers=[q])
    return Image.open("temp.jp2")


def webp_compression(image_path, image, quality):
    """Apply WebP compression."""
    image.save("temp.webp", "WEBP", quality=quality)
    return Image.open("temp.webp")


def apply_median_filter(image_path, image, size):
    """Apply Median filter to the image."""
    return image.filter(ImageFilter.MedianFilter(size=size))


def apply_average_filter(image_path, image):
    """Apply Average filter to the image."""
    return image.filter(ImageFilter.BLUR)


def pixel_elimination(image_path, image, elimination_ratio):
...
def cropout(image_path, image, min_crop_ratio, max_crop_ratio):
...
def copy_move_attack(image_path, image, min_patch_size, max_patch_size):
    """
    ...
    patch_size_y = int(height * patch_size_ratio)
    patch_size_x = int(width * patch_size_ratio)

    # Randomly select the top-left corner of the patch to copy
    src_y = np.random.randint(0, height - patch_size_y)
    src_x = np.random.randint(0, width - patch_size_x)

    # Randomly select the top-left corner of the location to paste the patch
    dst_y = np.random.randint(0, height - patch_size_y)
    dst_x = np.random.randint(0, width - patch_size_x)

    # Copy the patch
    patch = np_image[src_y:src_y + patch_size_y, src_x:src_x + patch_size_x]

    # Paste the patch at the new location
    np_image[dst_y:dst_y + patch_size_y, dst_x:dst_x + patch_size_x] = patch

    return Image.fromarray(np_image)


def inpainting_attack(image_path, image, min_patch_size, max_patch_size):
    """
    ...
    mask[src_y:src_y + patch_size_y, src_x:src_x + patch_size_x] = 1
    inpainted_image = cv2.inpaint(np_image, mask, inpaintRadius=3, flags=cv2.INPAINT_NS)
    return Image.fromarray(inpainted_image)

    # PyInpaint
    # mask = np.ones((height, width), dtype=np.uint8)
    # mask[src_y:src_y + patch_size, src_x:src_x + patch_size] = 0
    # mask_image = Image.fromarray(mask * 255)  # Multiply by 255 to scale 1s to 255 (white in binary images) / # Save the mask as a PNG file
    # mask_image.save("tmp.png")
    #
    # inpaint = Inpaint(image_path, 'tmp.png')
    # inpainted_image = inpaint()
    #
    # return Image.fromarray(inpainted_image)


def image_splicing_attack(image_path, image, min_patch_size=0.2, max_patch_size=0.4, directory="Mixed-Immune"):
    """
    ...
    # Randomly select the patch size as a portion of the image size
    patch_size_ratio = np.random.uniform(min_patch_size, max_patch_size)
    patch_size = int(min(rand_height, rand_width, height, width) * patch_size_ratio)
    patch_size_y = int(min(height, rand_height) * patch_size_ratio)
    patch_size_x = int(min(width, rand_width) * patch_size_ratio)

    # Randomly select the top-left corner of the patch to copy from the random image
    src_y = np.random.randint(0, rand_height - patch_size_y)
    src_x = np.random.randint(0, rand_width - patch_size_x)
    ...
    patch = np_random_image[src_y:src_y + patch_size_y, src_x:src_x + patch_size_x]
...
def cropping(image_path, image, min_crop_ratio=0.5, max_crop_ratio=0.9):
    """
...
def calculate_metrics(original, transformed, transformation_name):
    ...
    if re.match(r"^(resize_keep_ratio|resize_no_ratio|cropping|add_gaussian_noise|apply_gaussian_blur)", transformation_name[0]):  #  or re.match(r"^cropping", transformation_name[0])
...
def apply_transformations(image_path, common_transformations, attack_transformations, params, combine=False, attack=False):
    """
    ...
    if not attack:
        transform = random.sample(common_transformations, 2)
        for t in transform:
            param = params[t.__name__]
            image = t(image_path, image, **param)
            applied_transforms.append(f"{t.__name__}({param})")
    else:
        transform = random.sample(attack_transformations, 1)
        transform += random.sample(common_transformations, 2)
        for t in transform:
            param = params[t.__name__]
            image = t(image_path, image, **param)
    ...
    #         image = transform(image_path, image, **param)
...
def process_images(input_dir, output_dir, common_transformations, attack_transformations,  params, combine=False, attack=False):
    """
    ...
    with open(os.path.join(output_dir, 'report.csv'), mode='w', newline='') as file:
        ...
        for image_name in os.listdir(input_dir):
            ...
            if os.path.isfile(image_path) and os.path.splitext(image_name)[1].lower() in allowed_extensions:
                transformed_image, applied_transforms = apply_transformations(image_path, common_transformations, attack_transformations, params,
                                                                              combine, attack)
                if "jpeg_compression({'min_q': 70, 'max_q': 91})" in applied_transforms:
                    transformed_image_name = f"{os.path.splitext(image_name)[0]}_transformed.jpeg"
                else:
                    transformed_image_name = f"{os.path.splitext(image_name)[0]}_transformed.png"
                ...
                writer.writerow([image_name, transformed_image_name, transformations_applied])  # psnr_value, ssim_value, wasserstein_value
...
if __name__ == "__main__":
    ...
    common_transformations = [jpeg_compression, resize_keep_ratio, add_gaussian_noise, apply_gaussian_blur]
    # common_transformations = [[jpeg_compression], [resize_keep_ratio], [add_gaussian_noise], [apply_gaussian_blur]]


    attack_transformations = [cropping, copy_move_attack, image_splicing_attack, inpainting_attack]
    # attack_transformations = [cropping]
    # attack_transformations = [copy_move_attack, image_splicing_attack]
    # attack_transformations = [inpainting_attack]

    Datasets = ['FFHQ']  # "MetFace", "DIV2K", "CLIC-F", "DIV2K"
    for data in Datasets:
        ...
        params = {
            ########## Authentic Transformations
            "jpeg_compression": {'min_q': 70, 'max_q': 91},
            # "jpeg2000_compression": {"min_db": 40, 'max_db': 60},
            "resize_keep_ratio": {"min_scale": 0.5, 'max_scale':0.9},
            "add_gaussian_noise": {"mean": 0, "min_std": 0.1, 'max_std': 0.5},
            "apply_gaussian_blur": {"min_radius": 0.1, "max_radius":0.5},

            ########## Inauthentic Transformations
            "cropping": {"min_crop_ratio": 0.5, "max_crop_ratio": 0.9},  # CC
            # "cropout": {"min_crop_ratio": 0.2, "max_crop_ratio": 0.4},  # CC
        ...
        for attack in attack_list:
            if attack:
                output_dir = data + "-transformed/" + str("attack")
                # output_dir = data + "-transformed/" + str("obj-remove")
            else:
                output_dir = data + "-transformed/" + str("common")
            # Apply transformations (set combine=True to apply all transformations together)
            process_images(input_dir, output_dir, common_transformations, attack_transformations, params, combine=True, attack=attack)
