...
def extract_watermark(image_path):
    ...
    extracted_hash_str, wm_present, _, _ = tm.decode(stego_image=image, MODE='binary')
    if wm_present:
        return extracted_hash_str[:-4], True
    else:
        return extracted_hash_str[:-4], False
...
def verify_image(image_path):
    ...
    """
    computed_hash = compute_phash(image_path=image_path, hash_size=8)
