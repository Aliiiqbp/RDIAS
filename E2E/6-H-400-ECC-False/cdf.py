...
def plot_cdf(hamming_distances, color, label, threshold):
    ...
    print(f"CDF for {label} at threshold {threshold}: {cdf_at_threshold:.4f}")
...
thresholds = [i for i in range(0, 20)]

for threshold in thresholds:
    for data in Datasets:
        print(data, threshold)    # Directories containing the CSV files    # Directories containing the CSV files
